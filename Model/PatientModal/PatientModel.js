...
const createPatient = async(patientData, ghlContactId) => {
    try {
        const query = "INSERT INTO patients (ghl_contact_id, patient_lead_source, banned, patient_lead_owner, patient_lead_name, last_name, phone, phone2, email, dob, age, height, weight, habits, medication, diagnosis, surgeries, status, created_by, modified_by, qualified_status, dnq, not_interested_reason) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
            ghlContactId || null,
...
const updatePatient = async(id, patientData) => {
    try {
        const query = "UPDATE patients SET patient_lead_source = ?, banned = ?, patient_lead_owner = ?, patient_lead_name = ?, last_name = ?, phone = ?, phone2 = ?, email = ?, dob = ?, age = ?, height = ?, weight = ?, habits = ?, medication = ?, diagnosis = ?, surgeries = ?, status = ?, modified_by = ?, qualified_status = ?, dnq = ?, not_interested_reason = ? WHERE patient_id = ?";
        const values = [
            patientData.patientLeadSource || null,
            patientData.banned || null,
            patientData.patientOwner || null,
